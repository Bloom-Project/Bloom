...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a BLM address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no BLM: URI
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient* out)
{
    // Convert BLM:// to BLM:
    //
    //    Cannot handle this later, because BLM:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open BLM.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "BLM.lnk";
}
...
{
    return GetAutostartDir() / "BLM.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a BLM.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the BLM app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add BLM app to startup item list
