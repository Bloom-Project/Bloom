...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeBLMAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeBLMAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeBLMAmount = strAnonymizeBLMAmount.remove(strAnonymizeBLMAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBLMAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeBLMAmount * COIN) nMaxToAnonymize = nAnonymizeBLMAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeBLMAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeBLMAmount));
        strAnonymizeBLMAmount = strAnonymizeBLMAmount.remove(strAnonymizeBLMAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBLMAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
