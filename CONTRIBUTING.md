...
The BLM project operates an open contributor model where anyone is
...
The following applies to code changes to the BLM Core project, and is not to be
confused with overall BLM Network Protocol consensus changes.

Whether a pull request is merged into BLM Core rests with the project merge
...
Patches that change BLM consensus rules are considerably more involved than
...
Where a patch set proposes to change the BLM consensus, it must have been
...
The project leader is the release manager for each BLM Core release.
